...
- **Customers → report column order (drag & drop)** (`components/CustomerReportModal.tsx`, `api/routers/customer_reports.py`): the PDF/CSV report modal's column picker is an ordered list instead of a checkbox grid. Dragging a row (or its up/down buttons — touch has no HTML5 drag, and a keyboard user has no pointer) sets where that column prints, top of the list being the leftmost column; each ticked row shows its print position and a summary line spells the order out. `_pick_columns()` now preserves the caller's order rather than re-sorting into `_COLUMNS` registry order — an empty or fully-invalid `columns` list still falls back to every column in registry order, so plain `GET /api/reports/customers.csv` is byte-identical to before.
- **Customers → list sort dropdown** (`app/(app)/customers/page.tsx`, `api/routers/customers.py`): `GET /api/customers` takes `sort=name|start_date|price`, applied **before** the page slice — sorting the 25 rows of the current page would reorder a page, not the list. `start_date` and `price` are newest/highest first with customers who have no rental yet sinking to the bottom, and `full_name` breaks every tie so paging is stable. One dropdown drives both views (the card deck's full-list fetch and the paged table); the choice persists in `localStorage`. An unknown `sort=` falls back to A–Z.
- **Fixed — deleted records came back** (`lib/deleteUndo.tsx`, `lib/api.ts`): a confirmed delete was never actually sent. `settle()` handed the pending entry back through a `setPending` updater, which React 18 runs during the *next* render, so the 10s timer's `if (!toCommit) return` always fired first and the DELETE was skipped — the row vanished locally and reappeared on the next load. The pending list is now settled synchronously through a ref. Two related holes closed with it: `pagehide` flushes still-pending deletes (`keepalive`, so the request outlives the document) instead of dropping them on a refresh, and callers pass a `key` so a background poll landing mid-window cannot re-add a row through `isPending()`.
